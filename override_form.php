...
/**
 * Settings form for overrides in the quizinvideo module.
 *
...
require_once($CFG->dirroot . '/mod/quizinvideo/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizinvideo_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the quizinvideo settings object. */
     ...
     * @param object the quizinvideo settings object.
     ...
     */
    public function __construct($submiturl, $cm, $quizinvideo, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'quizinvideo'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'quizinvideo'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/quizinvideo/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'quizinvideo', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'quizinvideo'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/quizinvideo:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/quizinvideo/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'quizinvideo', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'quizinvideo'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'quizinvideo');
        $mform->setDefault('password', $this->quizinvideo->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('quizinvideoopen', 'quizinvideo'), mod_quizinvideo_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->quizinvideo->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('quizinvideoclose', 'quizinvideo'), mod_quizinvideo_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->quizinvideo->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'quizinvideo'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'quizinvideo');
        $mform->setDefault('timelimit', $this->quizinvideo->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'quizinvideo'), $attemptoptions);
        $mform->setDefault('attempts', $this->quizinvideo->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'quizinvideo'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'quizinvideo'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'quizinvideo'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one quizinvideo setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $quizinvideo->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'quizinvideo');
        }
