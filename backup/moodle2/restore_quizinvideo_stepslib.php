...
class restore_quizinvideo_activity_structure_step extends restore_questions_activity_structure_step {
    ...
    /**
     * @var bool tracks whether the quizinvideo contains at least one section. Before
     * Moodle 2.9 quizinvideo sections did not exist, so if the file being restored
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('quizinvideo_section', '/activity/quizinvideo/sections/section');
    ...
    protected function process_quizinvideo_section($data) {
        global $DB;

        $data = (object) $data;
        $data->quizinvideoid = $this->get_new_parentid('quizinvideo');
        $newitemid = $DB->insert_record('quizinvideo_sections', $data);
    ...
    protected function after_execute() {
        ...
        if (!$this->sectioncreated) {
            $DB->insert_record('quizinvideo_sections', array(
                    'quizinvideoid' => $this->get_new_parentid('quizinvideo'),
                    'firstslot' => 1, 'heading' => '',
