...
function quizinvideo_start_new_attempt($quizinvideoobj, $quba, $attempt, $attemptnumber, $timenow,
    ...
    // Usages for this user's previous quizinvideo attempts.
    $qubaids = new \mod_quizinvideo\question\qubaids_for_users_attempts(
            $quizinvideoobj->get_quizinvideoid(), $attempt->userid);
    ...
    if ($randomfound) {
        ...
        foreach ($quizinvideoobj->get_questions() as $questiondata) {
            ...
            if (isset($questionids[$quba->next_slot_number()])) {
                if ($randomloader->is_question_available($questiondata->category,
                    ...
                    $questions[$slot] = question_bank::load_question(
                            $questionids[$quba->next_slot_number()], $quizinvideoobj->get_quizinvideo()->shuffleanswers);
                    continue;
                } else {
            ...
            $questions[$slot] = question_bank::load_question($questionid,
                    $quizinvideoobj->get_quizinvideo()->shuffleanswers);
        }
    }
    ...
    $sections = $quizinvideoobj->get_sections();
...
function quizinvideo_repaginate_questions($quizinvideoid, $slotsperpage) {
    ...
    $sections = $DB->get_records('quizinvideo_sections', array('quizinvideoid' => $quizinvideoid), 'firstslot ASC');
    $firstslots = array();
    foreach ($sections as $section) {
    ...
    if (is_int($page) && $page >= 1) {
        ...
        $DB->execute("
                UPDATE {quizinvideo_sections}
                 ...
                 WHERE quizinvideoid = ?
                   AND firstslot > ?
                ", array($quizinvideo->id, max($lastslotbefore, 1)));
