...
$cm = get_coursemodule_from_id('quizinvideo', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$quizinvideo = $DB->get_record('quizinvideo', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = quizinvideo_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see quizinvideo reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/quizinvideo/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = quizinvideo_get_user_attempts($quizinvideo->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new quizinvideo_attempt($attempt, $quizinvideo, $cm, $course, false);
...
redirect(new moodle_url('/mod/quizinvideo/report.php', array(
