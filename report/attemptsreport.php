...
/**
 * Base class for quizinvideo reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class quizinvideo_attempts_report extends quizinvideo_default_report {
    ...
    /** @var object the quizinvideo context. */
    protected $context;

    /** @var mod_quizinvideo_attempts_report_form The settings form to use. */
    protected $form;
     ...
     * @param object $course
     */
    protected function init($mode, $formclass, $quizinvideo, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = quizinvideo_report_qm_filter_select($quizinvideo);

        $this->form = new $formclass($this->get_base_url(),
                array('quizinvideo' => $quizinvideo, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'quizinvideo');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'quizinvideo');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'quizinvideo');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'quizinvideo');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this quizinvideo.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($quizinvideo, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'quizinvideo') . '/' .
                    quizinvideo_format_grade($quizinvideo, $quizinvideo->grade);
        }

        if ($includefeedback && quizinvideo_has_feedback($quizinvideo)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'quizinvideo');
        }
    }
     ...
     * @param mod_quizinvideo_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_quizinvideo_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $groupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/quizinvideo:deleteattempts', $this->context);
                    $this->delete_selected_attempts($quizinvideo, $cm, $attemptids, $allowed);
    ...
    /**
     * Delete the quizinvideo attempts
     * @param object $quizinvideo the quizinvideo settings. Attempts that don't belong to
     * this quizinvideo are not deleted.
     ...
     */
    protected function delete_selected_attempts($quizinvideo, $cm, $attemptids, $allowed) {
        global $DB;

        foreach ($attemptids as $attemptid) {
            $attempt = $DB->get_record('quizinvideo_attempts', array('id' => $attemptid));
            if (!$attempt || $attempt->quizinvideo != $quizinvideo->id || $attempt->preview != 0) {
                // Ensure the attempt exists, and belongs to this quizinvideo. If not skip.
                continue;
            }
            ...
            // Set the course module id before calling quizinvideo_delete_attempt().
