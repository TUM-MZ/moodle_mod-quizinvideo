...
/**
 * This file defines the quizinvideo manual grading report class.
 *
...
require_once($CFG->dirroot . '/mod/quizinvideo/report/grading/gradingsettings_form.php');


/**
 * quizinvideo report to help teachers manually grade questions that need it.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizinvideo_grading_report extends quizinvideo_default_report {
    ...
    public function display($quizinvideo, $cm, $course) {
        ...
        require_capability('mod/quizinvideo:grade', $this->context);
        $shownames = has_capability('quizinvideo/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('quizinvideo/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this quizinvideo.
        $this->questions = quizinvideo_report_get_significant_questions($quizinvideo);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'quizinvideo_grading');
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'quizinvideo_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $quizinvideo, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "quizinvideoa.quizinvideo = :mangrquizinvideoid AND
                quizinvideoa.preview = 0 AND
                quizinvideoa.state = :statefinished";
        $params = array('mangrquizinvideoid' => $this->cm->instance, 'statefinished' => quizinvideo_attempt::FINISHED);

        $currentgroup = groups_get_activity_group($this->cm, true);
        if ($currentgroup) {
            $users = get_users_by_capability($this->context,
                    array('mod/quizinvideo:reviewmyattempts', 'mod/quizinvideo:attempt'), 'u.id, u.id', '', '', '',
                    $currentgroup, '', false);
            if (empty($users)) {
                $where .= ' AND quizinvideoa.userid = 0';
            } else {
        ...
        return new qubaid_join('{quizinvideo_attempts} quizinvideoa', 'quizinvideoa.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = quizinvideo_attempt::FINISHED;
        $params[] = $this->quizinvideo->id;

        $fields = 'quizinvideoa.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {quizinvideo_attempts} quizinvideoa
                JOIN {user} u ON u.id = quizinvideoa.userid
                WHERE quizinvideoa.uniqueid $asql AND quizinvideoa.state = ? AND quizinvideoa.quizinvideo = ?",
                $params);
    ...
    protected function base_url() {
        return new moodle_url('/mod/quizinvideo/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'quizinvideo_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'quizinvideo_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'quizinvideo_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'quizinvideo_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'quizinvideo_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'quizinvideo_grading');
        $table->head[] = get_string('questionname', 'quizinvideo_grading');
        $table->head[] = get_string('tograde', 'quizinvideo_grading');
        $table->head[] = get_string('alreadygraded', 'quizinvideo_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'quizinvideo_grading');
        }
        $table->head[] = get_string('total', 'quizinvideo_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new quizinvideo_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'quizinvideo_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'quizinvideo_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'quizinvideo_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = quizinvideo_get_review_options($this->quizinvideo, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'value' => get_string('saveandnext', 'quizinvideo_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'quizinvideo_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'quizinvideo_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new quizinvideo_attempt($attempt, $this->quizinvideo, $this->cm, $this->course);
            ...
            $events[] = \mod_quizinvideo\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON quizinvideoa.userid = u.id ";
