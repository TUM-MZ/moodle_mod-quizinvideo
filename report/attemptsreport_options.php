...
/**
 * Base class for the options that control what is visible in an {@link quizinvideo_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizinvideo_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the quizinvideo being reported on. */
    public $quizinvideo;

    /** @var object the course module objects for the quizinvideo being reported on. */
    public $cm;

    /** @var object the course settings for the course the quizinvideo is in. */
    public $course;

    /**
     * @var array form field name => corresponding quizinvideo_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => quizinvideo_attempt::IN_PROGRESS,
        'stateoverdue'    => quizinvideo_attempt::OVERDUE,
        'statefinished'   => quizinvideo_attempt::FINISHED,
        'stateabandoned'  => quizinvideo_attempt::ABANDONED,
    );

    /**
     * @var string quizinvideo_attempts_report::ALL_WITH or quizinvideo_attempts_report::ENROLLED_WITH
     *      quizinvideo_attempts_report::ENROLLED_WITHOUT or quizinvideo_attempts_report::ENROLLED_ALL
     */
    public $attempts = quizinvideo_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of quizinvideo_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(quizinvideo_attempt::IN_PROGRESS, quizinvideo_attempt::OVERDUE,
            quizinvideo_attempt::FINISHED, quizinvideo_attempt::ABANDONED);
    ...
    public $pagesize = quizinvideo_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $quizinvideo the settings for the quizinvideo being reported on.
     * @param object $cm the course module objects for the quizinvideo being reported on.
     * @param object $coures the course settings for the coures this quizinvideo is in.
     */
    public function __construct($mode, $quizinvideo, $cm, $course) {
        $this->mode   = $mode;
        $this->quizinvideo   = $quizinvideo;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = quizinvideo_report_should_show_grades($quizinvideo, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/quizinvideo/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('quizinvideo_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('quizinvideo_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == quizinvideo_attempts_report::ALL_WITH) {
                $this->attempts = quizinvideo_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = quizinvideo_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(quizinvideo_attempts_report::ALL_WITH, quizinvideo_attempts_report::ENROLLED_WITH,
                quizinvideo_attempts_report::ENROLLED_WITHOUT, quizinvideo_attempts_report::ENROLLED_ALL))) {
            $this->attempts = quizinvideo_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!quizinvideo_report_can_filter_only_graded($this->quizinvideo)) {
        ...
        if ($this->attempts == quizinvideo_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = quizinvideo_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(quizinvideo_attempt::FINISHED, $this->states);
    }
}
