...
/**
 * Standard plugin entry points of the quizinvideo statistics report.
 *
 ...
 * @package  quizinvideo_statistics
 * @category files
 * @param context $previewcontext the quizinvideo context
 ...
 */
function quizinvideo_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('quizinvideo/statistics:view', $context);
...
/**
 * quizinvideo statistics report cron code. Deletes cached data more than a certain age.
 */
function quizinvideo_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old quizinvideo statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('quizinvideo_statistics', 'timemodified < ?', array($expiretime));
