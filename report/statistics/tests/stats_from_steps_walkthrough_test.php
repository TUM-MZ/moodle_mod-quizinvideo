 ...
 *
 * The quizinvideo stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-quizinvideo-tools/tree/master/statsspreadsheet
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizinvideo_report_statistics_from_steps_testcase extends mod_quizinvideo_attempt_walkthrough_from_csv_testcase {

    /**
     * @var quizinvideo_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    public function test_walkthrough_from_csv($quizinvideosettings, $csvdata) {
        ...
        $whichattempts = quizinvideo_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudents = array();
        list($questions, $quizinvideostats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents);
        if ($quizinvideosettings['testnumber'] === '00') {
            $this->check_variants_count_for_quizinvideo_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_quizinvideo_stats_for_quizinvideo_00($quizinvideostats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_quizinvideo_00($questions, $questionstats, $whichtries, $qubaids) {
    ...
    /**
     * @param $quizinvideostats
     */
    protected function check_quizinvideo_stats_for_quizinvideo_00($quizinvideostats) {
        $quizinvideostatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($quizinvideostatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $quizinvideostats->$statname, $quizinvideostats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $quizinvideostats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents) {
        $this->report = new quizinvideo_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->quizinvideo);
        list($quizinvideostats, $questionstats) = $this->report->get_all_stats_and_analysis($this->quizinvideo,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudents,
                                                                                     $questions);

        $qubaids = quizinvideo_statistics_qubaids_condition($this->quizinvideo->id, $groupstudents, $whichattempts);

        // We will create some quizinvideo and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $quizinvideocalc = new \quizinvideo_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($quizinvideocalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $quizinvideostats, $questionstats, $qubaids);
        }
        return array($questions, $quizinvideostats, $questionstats, $qubaids);
    }
