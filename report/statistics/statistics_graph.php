...
/**
 * This script renders the quizinvideo statistics graph.
 *
 * It takes one parameter, the quizinvideo_statistics.id. This is enough to identify the
 * quizinvideo etc.
 *
...
// Check access.
require_login($quizinvideo->course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('quizinvideo/statistics:view', $modcontext);
...
if (empty($currentgroup)) {
    ...
    $groupstudents = get_users_by_capability($modcontext, array('mod/quizinvideo:reviewmyattempts', 'mod/quizinvideo:attempt'),
                                             '', '', '', '', $currentgroup, '', false);
}
$qubaids = quizinvideo_statistics_qubaids_condition($quizinvideoid, $groupstudents, $whichattempts);
...
$graph->parameter['x_label'] = get_string('position', 'quizinvideo_statistics');
...
$fieldstoplot = array(
    ...
    'discriminativeefficiency' => get_string('discriminative_efficiency', 'quizinvideo_statistics')
);
...
foreach (array_keys($fieldstoplot) as $fieldtoplot) {
    ...
    $graph->y_format[$fieldtoplot] = array(
        'colour' => quizinvideo_statistics_graph_get_new_colour(),
        'bar' => 'fill',
        'shadow_offset' => 1,
