...
/**
 * Post-install script for the quizinvideo statistics report.
 *
...
/**
 * quizinvideo statistics report upgrade code.
 */
function xmldb_quizinvideo_statistics_upgrade($oldversion) {
    global $DB;
    ...
    if ($oldversion < 2013092000) {
        ...
        $table = new xmldb_table('quizinvideo_question_statistics');
        ...
        $table = new xmldb_table('quizinvideo_question_response_stats');
        ...
        $table = new xmldb_table('quizinvideo_statistics');
        $field = new xmldb_field('quizinvideoid');
        ...
        upgrade_plugin_savepoint(true, 2013092000, 'quizinvideo', 'statistics');
    }

    if ($oldversion < 2013093000) {
        // Define table quizinvideo_statistics to be dropped.
        $table = new xmldb_table('quizinvideo_statistics');

        // Conditionally launch drop table for quizinvideo_statistics.
        if ($dbman->table_exists($table)) {
            $dbman->drop_table($table);
        }

        // Define table quizinvideo_statistics to be created.
        $table = new xmldb_table('quizinvideo_statistics');

        // Adding fields to table quizinvideo_statistics.
        ...
        // Adding keys to table quizinvideo_statistics.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Conditionally launch create table for quizinvideo_statistics.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Statistics savepoint reached.
        upgrade_plugin_savepoint(true, 2013093000, 'quizinvideo', 'statistics');
    }
