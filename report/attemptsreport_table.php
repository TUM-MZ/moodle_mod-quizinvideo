...
/**
 * Base class for the table used by a {@link quizinvideo_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class quizinvideo_attempts_report_table extends table_sql {
    ...
    /** @var object the quizinvideo settings for the quizinvideo we are reporting on. */
    ...
    /** @var object mod_quizinvideo_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this quizinvideo.. */
    protected $questions;
     ...
     * @param mod_quizinvideo_attempts_report_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($uniqueid, $quizinvideo, $context, $qmsubselect,
            mod_quizinvideo_attempts_report_options $options, $groupstudents, $students,
            $questions, $reporturl) {
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return quizinvideo_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = quizinvideo_report_feedback_for_grade(
                quizinvideo_rescale_grade($attempt->sumgrades, $this->quizinvideo, false),
                $this->quizinvideo->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/quizinvideo/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'quizinvideo')));
    ...
    public function base_sql($reportstudents) {
        ...
        $fields = $DB->sql_concat('u.id', "'#'", 'COALESCE(quizinvideoa.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                quizinvideoa.uniqueid AS usageid,
                quizinvideoa.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                u.email' . $extrafields . ',
                quizinvideoa.state,
                quizinvideoa.sumgrades,
                quizinvideoa.timefinish,
                quizinvideoa.timestart,
                CASE WHEN quizinvideoa.timefinish = 0 THEN null
                     WHEN quizinvideoa.timefinish > quizinvideoa.timestart THEN quizinvideoa.timefinish - quizinvideoa.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and quizinvideo_attempts tables.
        $from = "\n{user} u";
        $from .= "\nLEFT JOIN {quizinvideo_attempts} quizinvideoa ON
                                    quizinvideoa.userid = u.id AND quizinvideoa.quizinvideo = :quizinvideoid";
        $params = array('quizinvideoid' => $this->quizinvideo->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (quizinvideoa.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = quizinvideo_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case quizinvideo_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'quizinvideoa.id IS NOT NULL AND quizinvideoa.preview = 0';
                break;
            case quizinvideo_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND quizinvideoa.preview = 0 AND quizinvideoa.id IS NOT NULL";
                break;
            case quizinvideo_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND quizinvideoa.id IS NULL";
                break;
            case quizinvideo_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND (quizinvideoa.preview = 0 OR quizinvideoa.preview IS NULL)";
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (quizinvideoa.state $statesql OR quizinvideoa.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{quizinvideo_attempts} {$alias}quizinvideoa", "{$alias}quizinvideoa.uniqueid",
                "{$alias}quizinvideoa.quizinvideo = :{$alias}quizinvideoid", array("{$alias}quizinvideoid" => $this->sql->params['quizinvideoid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = quizinvideoa.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join($this->sql->from, 'quizinvideoa.uniqueid',
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['quizinvideoa.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a href="javascript:select_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectall', 'quizinvideo') . '</a> / ';
        echo '<a href="javascript:deselect_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectnone', 'quizinvideo') . '</a> ';
        echo '&nbsp;&nbsp;';
        $this->submit_buttons();
        echo '</div>';
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/quizinvideo:deleteattempts', $this->context)) {
            echo '<input type="submit" id="deleteattemptsbutton" name="delete" value="' .
                    get_string('deleteselected', 'quizinvideo_overview') . '"/>';
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    array('message' => get_string('deleteattemptcheck', 'quizinvideo')));
        }
    }
}
