YUI.add('moodle-mod_quizinvideo-toolboxes', function (Y, NAME) {
            ...
            on: {
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(M.util.get_string('numquestionsx', 'quizinvideo', responsetext.newnumquestions));
                    }
                    if (success_callback) {
 ...
 * when viewing a course in editing mode.
 *
 * @module moodle-mod_quizinvideo-toolboxes
 * @namespace M.mod_quizinvideo.toolboxes
 */
 ...
 * @class section
 * @constructor
 * @extends M.mod_quizinvideo.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    toggle_hide_section : function(e) {
        ...
        var section = e.target.ancestor(M.mod_quizinvideo.format.get_section_selector(Y)),
        ...
        var data = {
            'class' : 'section',
            'field' : 'visible',
            'id'    : Y.Moodle.core_course.util.section.getId(section.ancestor(M.mod_quizinvideo.edit.get_section_wrapper(Y), true)),
            'value' : value
        };
        ...
        this.send_request(data, lightbox, function(response) {
            ...
            activities.each(function(node) {
                ...
                var activityid = Y.Moodle.mod_quizinvideo.util.slot.getId(node);

                // NOTE: resourcestotoggle is returned as a string instead
                // of a Number so we must cast our activityid to a String.
                if (Y.Array.indexOf(response.resourcestotoggle, "" + activityid) !== -1) {
                    M.mod_quizinvideo.resource_toolbox.handle_resource_dim(button, node, action);
                }
            }, this);
        });
    },
    ...
    toggle_highlight : function(e) {
        ...
        // Get the section we're working on.
        var section = e.target.ancestor(M.mod_quizinvideo.edit.get_section_selector(Y));
        ...
        Y.one(SELECTOR.PAGECONTENT)
            .all(M.mod_quizinvideo.edit.get_section_selector(Y) + '.current ' + SELECTOR.HIGHLIGHT)
            .set('title', old_string);
        Y.one(SELECTOR.PAGECONTENT)
            .all(M.mod_quizinvideo.edit.get_section_selector(Y) + '.current ' + SELECTOR.HIGHLIGHT + ' img')
            .set('alt', old_string)
            .set('src', M.util.image_url('i/marker'));

        // Remove the highlighting from all sections.
        Y.one(SELECTOR.PAGECONTENT).all(M.mod_quizinvideo.edit.get_section_selector(Y))
            .removeClass('current');

        // Then add it if required to the selected section.
        if (!togglestatus) {
            section.addClass('current');
            value = Y.Moodle.core_course.util.section.getId(section.ancestor(M.mod_quizinvideo.edit.get_section_wrapper(Y), true));
    ...
    NAME : 'mod_quizinvideo-section-toolbox',
    ATTRS : {
        courseid : {
            'value' : 0
        },
...
M.mod_quizinvideo.init_section_toolbox = function(config) {
