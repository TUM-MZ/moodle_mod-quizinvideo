...
/**
 * Steps definitions related to mod_quizinvideo.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_quizinvideo extends behat_question_base {

    /**
     * Put the specified questions on the specified pages of a given quizinvideo.
     *
     ...
     *
     * @param string $quizinvideoname the name of the quizinvideo to add questions to.
     * @param TableNode $data information about the questions to add.
     *
     * @Given /^quizinvideo "([^"]*)" contains the following questions:$/
     */
    public function quizinvideo_contains_the_following_questions($quizinvideoname, TableNode $data) {
        global $DB;

        $quizinvideo = $DB->get_record('quizinvideo', array('name' => $quizinvideoname), '*', MUST_EXIST);
        ...
        if (!in_array('question', $firstrow) && !in_array('page', $firstrow)) {
            if (count($firstrow) == 2) {
                ...
                throw new ExpectationException('When adding questions to a quizinvideo, you should give 2 or three 3 things: ' .
        ...
        foreach ($data->getHash() as $questiondata) {
            if (!array_key_exists('question', $questiondata)) {
                throw new ExpectationException('When adding questions to a quizinvideo, ' .
            ...
            // Add the question.
            quizinvideo_add_quizinvideo_question($questionid, $quizinvideo, $page, $maxmark);

            // Require previous.
            if (array_key_exists('requireprevious', $questiondata)) {
                if ($questiondata['requireprevious'] === '1') {
                    $slot = $DB->get_field('quizinvideo_slots', 'MAX(slot)', array('quizinvideoid' => $quizinvideo->id));
                    $DB->set_field('quizinvideo_slots', 'requireprevious', 1,
                            array('quizinvideoid' => $quizinvideo->id, 'slot' => $slot));
    ...
    /**
     * Put the specified section headings to start at specified pages of a given quizinvideo.
     *
     * The first row should be column names:
     ...
     *
     * @param string $quizinvideoname the name of the quizinvideo to add sections to.
     * @param TableNode $data information about the sections to add.
     *
     * @Given /^quizinvideo "([^"]*)" contains the following sections:$/
     */
    public function quizinvideo_contains_the_following_sections($quizinvideoname, TableNode $data) {
        global $DB;
        ...
        foreach ($data->getHash() as $rownumber => $sectiondata) {
            if (!array_key_exists('heading', $sectiondata)) {
                throw new ExpectationException('When adding sections to a quizinvideo, ' .
            ...
            if ($rownumber == 0) {
                $section = $DB->get_record('quizinvideo_sections', array('quizinvideoid' => $quizinvideo->id), '*', MUST_EXIST);
            } else {
            ...
            if ($rownumber == 0) {
                ...
                $DB->insert_record('quizinvideo_sections', $section);
            }
        }

        if ($section->firstslot > $DB->count_records('quizinvideo_slots', array('quizinvideoid' => $quizinvideo->id))) {
            throw new ExpectationException('The section firstslot must be less than the total number of slots in the quizinvideo.',
                    $this->getSession());
        }
    }

    /**
     * Adds a question to the existing quizinvideo with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the "(?P<quizinvideo_name_string>(?:[^"]|\\")*)" quizinvideo with:$/
     * @param string $questiontype
     * @param string $quizinvideoname
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_quizinvideo_with($questiontype, $quizinvideoname, TableNode $questiondata) {
        $quizinvideoname = $this->escape($quizinvideoname);
        $editquizinvideo = $this->escape(get_string('editquizinvideo', 'quizinvideo'));
        $quizinvideoadmin = $this->escape(get_string('pluginadministration', 'quizinvideo'));
        $addaquestion = $this->escape(get_string('addaquestion', 'quizinvideo'));
        ...
        return array_merge(array(
            new Given("I follow \"$quizinvideoname\""),
            new Given("I navigate to \"$editquizinvideo\" node in \"$quizinvideoadmin\""),
    ...
    /**
     * Set the max mark for a question on the Edit quizinvideo page.
     *
     ...
     */
    public function i_set_the_max_mark_for_quizinvideo_question($questionname, $newmark) {
        return array(
            new Given('I follow "' . $this->escape(get_string('editmaxmark', 'quizinvideo')) . '"'),
    ...
    /**
     * Open the add menu on a given page, or at the end of the Edit quizinvideo page.
     * @Given /^I open the "(?P<page_n_or_last_string>(?:[^"]|\\")*)" add to quizinvideo menu$/
     * @param string $pageorlast either "Page n" or "last".
     */
    public function i_open_the_add_to_quizinvideo_menu_for($pageorlast) {
        ...
        if ($pageorlast == 'last') {
            ...
            throw new ExpectationException("The I open the add to quizinvideo menu step must specify either 'Page N' or 'last'.");
        }
    ...
    /**
     * Check whether a particular question is on a particular page of the quizinvideo on the Edit quizinvideo page.
     * @Given /^I should see "(?P<question_name>(?:[^"]|\\")*)" on quizinvideo page "(?P<page_number>\d+)"$/
     ...
     * @return array of steps.
     */
    public function i_should_see_on_quizinvideo_page($questionname, $pagenumber) {
    ...
    /**
     * Check whether a particular question is not on a particular page of the quizinvideo on the Edit quizinvideo page.
     * @Given /^I should not see "(?P<question_name>(?:[^"]|\\")*)" on quizinvideo page "(?P<page_number>\d+)"$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $pagenumber the page it should be found on.
     * @return array of steps.
     */
    public function i_should_not_see_on_quizinvideo_page($questionname, $pagenumber) {
        $xpath = "//li[contains(., '" . $this->escape($questionname) .
                "')][./preceding-sibling::li[contains(@class, 'pagenumber')][1][contains(., 'Page " .
                $pagenumber . "')]]";
        return array(
    ...
    /**
     * Check whether one question comes before another on the Edit quizinvideo page.
     * The two questions must be on the same page.
     * @Given /^I should see "(?P<first_q_name>(?:[^"]|\\")*)" before "(?P<second_q_name>(?:[^"]|\\")*)" on the edit quizinvideo page$/
     ...
     * @return array of steps.
     */
    public function i_should_see_before_on_the_edit_quizinvideo_page($firstquestionname, $secondquestionname) {
    ...
    /**
     * Check the number displayed alongside a question on the Edit quizinvideo page.
     * @Given /^"(?P<question_name>(?:[^"]|\\")*)" should have number "(?P<number>(?:[^"]|\\")*)" on the edit quizinvideo page$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $number the number (or 'i') that should be displayed beside that question.
     * @return array of steps.
     */
    public function should_have_number_on_the_edit_quizinvideo_page($questionname, $number) {
     ...
     *
     * @Given /^I click on shuffle for section "([^"]*)" on the quizinvideo edit page$/
     */
     ...
     *
     * @Given /^shuffle for section "([^"]*)" should be "(On|Off)" on the quizinvideo edit page$/
     */
    public function shuffle_for_section_should_be($heading, $value) {
        ...
        if ($value == 'On' && !$checkbox->isChecked()) {
            $msg = "Shuffle for section '$heading' is not checked, but you are expecting it to be checked ($value). " .
                    "Check the line with: \nshuffle for section \"$heading\" should be \"$value\" on the quizinvideo edit page" .
    ...
    /**
     * Move a question on the Edit quizinvideo page by first clicking on the Move icon,
     * then clicking one of the "After ..." links.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the quizinvideo by clicking the move icon$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Move a question on the Edit quizinvideo page by dragging a given question on top of another item.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the quizinvideo by dragging$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Delete a question on the Edit quizinvideo page by first clicking on the Delete icon,
     * then clicking one of the "After ..." links.
     * @When /^I delete "(?P<question_name>(?:[^"]|\\")*)" in the quizinvideo by clicking the delete icon$/
     * @param string $questionname the name of the question we are looking for.
     * @return array of steps.
     */
    ...
    /**
     * Set the section heading for a given section on the Edit quizinvideo page
     *
     * @When /^I change quizinvideo section heading "(?P<section_name_string>(?:[^"]|\\")*)" to "(?P<new_section_heading_string>(?:[^"]|\\")*)"$/
     ...
     * quizinvideo navigation block.
     *
     * @Then /^I should see question "(?P<questionnumber>\d+)" in section "(?P<section_heading_string>(?:[^"]|\\")*)" in the quizinvideo navigation$/
     ...
     */
    public function i_should_see_question_in_section_in_the_quizinvideo_navigation($questionnumber, $sectionheading) {
        ...
        $exception = new ExpectationException('Question "' . $questionnumber . '" is not in section "' .
                $sectionheading . '" in the quizinvideo navigation.', $this->getSession());
        $xpath = "//div[@id = 'mod_quizinvideo_navblock']//*[contains(concat(' ', normalize-space(@class), ' '), ' qnbutton ') and " .
