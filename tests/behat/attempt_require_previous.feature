...
Feature: Attemp a quizinvideo where some questions require that the previous question has been answered.
  In order to complete a quizinvideo where questions require previous ones to be complete
  As a student
  ...
  Scenario: A question that requires the previous one is initally blocked
    ...
    And the following "activities" exist:
      ...
      | quizinvideo       | quizinvideo 1 | quizinvideo 1 description | C1     | quizinvideo1    | immediatefeedback  |
  ...
  @javascript
  Scenario: After quizinvideo submitted, all questions show on the review page
    Given the following "questions" exist:
      | questioncategory | qtype       | name  | questiontext    |
      | Test questions   | truefalse   | TF1   | First question  |
      | Test questions   | truefalse   | TF2   | Second question |
  ...
  @javascript
  Scenario: A questions cannot be blocked in a deferred feedback quizinvideo (despite what is set in the DB).
    Given the following "questions" exist:
      | questioncategory | qtype       | name  | questiontext    |
      | Test questions   | truefalse   | TF1   | First question  |
      | Test questions   | truefalse   | TF2   | Second question |
  ...
  Scenario: Questions cannot be blocked in a shuffled section (despite what is set in the DB).
    ...
    And the following "activities" exist:
      ...
      | quizinvideo       | quizinvideo 1 | quizinvideo 1 description | C1     | quizinvideo1    | immediatefeedback  | 2                |
    And quizinvideo "quizinvideo 1" contains the following questions:
      | question | page | requireprevious |
      | TF1      | 1    | 1               |
      | TF2      | 2    | 1               |
  ...
  @javascript
  Scenario: A questions cannot be blocked in sequential quizinvideo (despite what is set in the DB).
    Given the following "questions" exist:
      | questioncategory | qtype       | name  | questiontext    |
      | Test questions   | truefalse   | TF1   | First question  |
      | Test questions   | truefalse   | TF2   | Second question |
