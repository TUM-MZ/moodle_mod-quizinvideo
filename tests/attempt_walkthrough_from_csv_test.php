 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizinvideo_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the quizinvideo record we create.
     */
    ...
    public function test_walkthrough_from_csv($quizinvideosettings, $csvdata) {
        ...
        // https://github.com/jamiepratt/moodle-quizinvideo-tools/tree/master/responsegenerator

        $this->create_quizinvideo_simulate_attempts_and_check_results($quizinvideosettings, $csvdata);
    }

    public function create_quizinvideo($quizinvideosettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $quizinvideosettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->quizinvideo = $quizinvideogenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                quizinvideo_add_quizinvideo_question($slotquestion['id'], $this->quizinvideo, 0, $slotquestion['mark']);
            } else {
                quizinvideo_add_random_questions($this->quizinvideo, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create quizinvideo, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $quizinvideosettings quizinvideo overrides for this quizinvideo.
     * @param PHPUnit_Extensions_Database_DataSet_ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_quizinvideo_simulate_attempts_and_check_results($quizinvideosettings, $csvdata) {
        ...
        for ($rowno = 0; $rowno < $quizinvideos->getRowCount(); $rowno++) {
            ...
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $quizinvideosettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $quizinvideosettings['testnumber']);
                }
            }
            $datasets[] = array($quizinvideosettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the quizinvideo_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the quizinvideo.
            ...
            if (!isset($attemptids[$step['quizinvideoattempt']])) {
                // Start the attempt.
                $quizinvideoobj = quizinvideo::create($this->quizinvideo->id, $user->id);
                ...
                $prevattempts = quizinvideo_get_user_attempts($this->quizinvideo->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = quizinvideo_create_attempt($quizinvideoobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                quizinvideo_start_new_attempt($quizinvideoobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                quizinvideo_attempt_save_started($quizinvideoobj, $quba, $attempt);
                $attemptid = $attemptids[$step['quizinvideoattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['quizinvideoattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the quizinvideo_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            $attemptobj = quizinvideo_attempt::create($attemptids[$result['quizinvideoattempt']]);
     ...
     * @param quizinvideo_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['quizinvideoattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['quizinvideoattempt']}.");
                    break;
                case 'quizinvideograde' :
                    // Check quizinvideo grades.
                    $grades = quizinvideo_get_user_grades($attemptobj->get_quizinvideo(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "quizinvideo grade for attempt {$result['quizinvideoattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(),
                                                        'mod', 'quizinvideo',
                                                        $attemptobj->get_quizinvideoid(),
                                                        $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['quizinvideoattempt']}.");
                    break;
                default :
