...
/**
 * Unit tests for the {@link \mod_quizinvideo\repaginate} class.
...
require_once($CFG->dirroot . '/mod/quizinvideo/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_quizinvideo\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizinvideo_repaginate_testable extends \mod_quizinvideo\repaginate {

    public function __construct($quizinvideoid = 0, $slots = null) {
        return parent::__construct($quizinvideoid, $slots);
    }
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizinvideo_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $quizinvideoslots;
    /** @var mod_quizinvideo_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_quizinvideo_slots($this->get_quizinvideo_object()->get_slots());
        $this->repaginate = new mod_quizinvideo_repaginate_testable(0, $this->quizinvideoslots);
    }
    ...
    /**
     * Create a quizinvideo, add five questions to the quizinvideo
     * which are all on one page and return the quizinvideo object.
     */
    private function get_quizinvideo_object() {
        global $SITE;
        ...
        $quizinvideo = $quizinvideogenerator->create_instance(array(
        ...
        quizinvideo_add_quizinvideo_question($shortanswer->id, $quizinvideo);
        quizinvideo_add_quizinvideo_question($numerical->id, $quizinvideo);
        quizinvideo_add_quizinvideo_question($essay->id, $quizinvideo);
        quizinvideo_add_quizinvideo_question($truefalse->id, $quizinvideo);
        quizinvideo_add_quizinvideo_question($match->id, $quizinvideo);

        // Return the quizinvideo object.
        $quizinvideoobj = new quizinvideo($quizinvideo, $cm, $SITE);
        return \mod_quizinvideo\structure::create_for_quizinvideo($quizinvideoobj);
    }

    /**
     * Set the quizinvideo slots
     * @param string $slots
     */
    private function set_quizinvideo_slots($slots = null) {
        if (!$slots) {
            $this->quizinvideoslots = $this->get_quizinvideo_object()->get_slots();
        } else {
            $this->quizinvideoslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_quizinvideo_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->quizinvideoslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->quizinvideoslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->quizinvideoslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->quizinvideoslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->quizinvideoslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizinvideoslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizinvideoslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizinvideoslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizinvideoslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizinvideoslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_quizinvideo_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->quizinvideoslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->quizinvideoslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->quizinvideoslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
