...
$PAGE->set_url('/mod/quizinvideo/comment.php', array('attempt' => $attemptid, 'slot' => $slot));

$attemptobj = quizinvideo_attempt::create($attemptid);

// Can only grade finished attempts.
if (!$attemptobj->is_finished()) {
    print_error('attemptclosed', 'quizinvideo');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/quizinvideo:grade');
...
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_quizinvideo.init_comment_popup', null, false, quizinvideo_get_js_module());
