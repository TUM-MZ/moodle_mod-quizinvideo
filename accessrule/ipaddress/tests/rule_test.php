...
/**
 * Unit tests for the quizinvideoaccess_ipaddress plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/quizinvideo/accessrule/ipaddress/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizinvideoaccess_ipaddress_testcase extends basic_testcase {
    public function test_ipaddress_access_rule() {
        ...
        $quizinvideo->subnet = getremoteaddr(null);
        if (!empty($quizinvideo->subnet)) {
            $quizinvideoobj = new quizinvideo($quizinvideo, $cm, null);
            $rule = new quizinvideoaccess_ipaddress($quizinvideoobj, 0);
