...
/**
 * Defines the custom question bank view used on the Edit quizinvideo page.
 *
...
namespace mod_quizinvideo\question\bank;
defined('MOODLE_INTERNAL') || die();


/**
 * Subclass to customise the view of the question bank for the quizinvideo editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the quizinvideo this is used by has been attemptd. */
    protected $quizinvideohasattempts = false;
    /** @var \stdClass the quizinvideo settings. */
    protected $quizinvideo = false;
     ...
     * @param \stdClass $quizinvideo quizinvideo settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $quizinvideo) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->quizinvideoquestionbankcolumns)) {
            $quizinvideoquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $quizinvideoquestionbankcolumns = explode(',', $CFG->quizinvideoquestionbankcolumns);
        }

        foreach ($quizinvideoquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_quizinvideo\\question\\bank\\' . $fullname)) {
                    $fullname = 'mod_quizinvideo\\question\\bank\\' . $fullname;
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_quizinvideo\\question\\bank\\' .
    ...
    protected function heading_column() {
        return 'mod_quizinvideo\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_quizinvideo\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the quizinvideo has been attempted,
     * hence whether some bits of UI, like the add this question to the quizinvideo icon,
     * should be displayed.
     * @param bool $quizinvideohasattempts whether the quizinvideo has attempts.
     */
    public function set_quizinvideo_has_attempts($quizinvideohasattempts) {
        $this->quizinvideohasattempts = $quizinvideohasattempts;
        if ($quizinvideohasattempts && isset($this->visiblecolumns['addtoquizinvideoaction'])) {
            unset($this->visiblecolumns['addtoquizinvideoaction']);
        }
    }

    public function preview_question_url($question) {
        return quizinvideo_question_preview_url($this->quizinvideo, $question);
    }

    public function add_to_quizinvideo_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/quizinvideo/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->quizinvideohasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the quizinvideo.
            $params = array(
                    'type' => 'submit',
                    'name' => 'add',
                    'value' => get_string('addselectedquestionstoquizinvideo', 'quizinvideo'),
            );
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/mod/quizinvideo/edit.php',
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'quizinvideo');
