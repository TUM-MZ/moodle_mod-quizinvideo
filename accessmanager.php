...
/**
 * Classes to enforce the various access rules that can apply to a quizinvideo.
 *
 ...
 * quizinvideo, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class quizinvideo_access_manager {
    /** @var quizinvideo the quizinvideo settings object. */
    protected $quizinvideoobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of quizinvideo_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular quizinvideo.
     * @param object $quizinvideoobj An instance of the class quizinvideo from attemptlib.php.
     *      The quizinvideo we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/quizinvideo:ignoretimelimits', ...)).
     */
    public function __construct($quizinvideoobj, $timenow, $canignoretimelimits) {
        $this->quizinvideoobj = $quizinvideoobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($quizinvideoobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular quizinvideo.
     * @param quizinvideo $quizinvideoobj information about the quizinvideo in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/quizinvideo:ignoretimelimits capability.
     * @return array of {@link quizinvideo_access_rule_base}s.
     */
    protected function make_rules($quizinvideoobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($quizinvideoobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['quizinvideoaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('quizinvideoaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the quizinvideo table.
     *
     * @param mod_quizinvideo_mod_form $quizinvideoform the quizinvideo settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_quizinvideo_mod_form $quizinvideoform, MoodleQuickForm $mform) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($quizinvideoform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'quizinvideo'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_quizinvideo_mod_form $quizinvideoform the quizinvideo form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_quizinvideo_mod_form $quizinvideoform) {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $quizinvideoform);
        }
    ...
    /**
     * Save any submitted settings when the quizinvideo settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the quizinvideo table.
     *
     * @param object $quizinvideo the data from the quizinvideo form, including $quizinvideo->id
     *      which is the id of the quizinvideo being saved.
     */
    public static function save_settings($quizinvideo) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($quizinvideo);
        }
    }

    /**
     * Delete any rule-specific settings when the quizinvideo is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the quizinvideo table.
     *
     * @param object $quizinvideo the data from the database, including $quizinvideo->id
     *      which is the id of the quizinvideo being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($quizinvideo) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($quizinvideo);
        }
    }
     ...
     */
    protected static function get_load_sql($quizinvideoid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{quizinvideo} quizinvideo';
        $allparams = array('quizinvideoid' => $quizinvideoid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($quizinvideoid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE quizinvideo.id = :quizinvideoid", $allparams);
    }
     ...
     */
    public static function load_settings($quizinvideoid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($quizinvideoid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($quizinvideoid);
        }
    ...
    /**
     * Load the quizinvideo settings and any settings required by the access rules.
     ...
     *
     * @param int $quizinvideoid the quizinvideo id.
     * @return object mdl_quizinvideo row with extra fields.
     */
    public static function load_quizinvideo_and_settings($quizinvideoid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($quizinvideoid, $rules, 'quizinvideo.*');
        $quizinvideo = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($quizinvideoid) as $name => $value) {
                $quizinvideo->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this quizinvideo, such
     * as is shown at the top of the quizinvideo view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this quizinvideo now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_quizinvideo_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        return new mod_quizinvideo_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'quizinvideoobj' => $this->quizinvideoobj,
     ...
     * quizinvideo. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant quizinvideo_attempts row.
    ...
    /**
     * @return bolean if this quizinvideo should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the quizinvideo view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_quizinvideo_renderer $output the quizinvideo renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->quizinvideoobj->view_url(), $message);
            die();
        } else {
            redirect($this->quizinvideoobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the quizinvideo, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(quizinvideo_attempt::IN_PROGRESS, quizinvideo_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = quizinvideo_attempt_state($this->quizinvideoobj->get_quizinvideo(), $attempt);
        $reviewoptions = mod_quizinvideo_display_options::make_from_quizinvideo(
                $this->quizinvideoobj->get_quizinvideo(), $when);
