...
/**
 * Fallback page of /mod/quizinvideo/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $quizinvideo, $pagevars) =
        question_edit_setup('editq', '/mod/quizinvideo/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/quizinvideo/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new quizinvideo_add_random_form(new moodle_url('/mod/quizinvideo/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    quizinvideo_add_random_questions($quizinvideo, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories);
...
// Setup $PAGE.
$streditingquizinvideo = get_string('editinga', 'moodle', get_string('modulename', 'quizinvideo'));
$PAGE->navbar->add($streditingquizinvideo);
$PAGE->set_title($streditingquizinvideo);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$quizinvideoname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontoquizinvideo', 'quizinvideo', $quizinvideoname), 2);
$mform->display();
echo $OUTPUT->footer();
